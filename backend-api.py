...
# ИСПРАВЛЕННАЯ ВЕРСИЯ - всё работает корректно!
...
# ============ PAYMENTS ENDPOINTS (FIXED!) ============

@app.post("/api/payments/send-invoice")
async def send_invoice(request: SendInvoiceRequest):
    """Send Telegram Stars invoice - IMPROVED VERSION"""
    try:
        telegram_id = request.telegram_id
        plan_input = request.plan.lower().strip() if request.plan else ""
        
        print(f"🔍 Payment DEBUG: Input plan = '{plan_input}'")
        
        # Map all possible plan names to standard format
        plan_mapping = {
            # LITE variants
            "lite": "lite",
            "лайт": "lite",
        ...
        # Normalize plan name
        normalized_plan = plan_mapping.get(plan_input, None)
        
        if not normalized_plan:
            available_plans = ", ".join(list(dict.fromkeys(plan_mapping.values())))
            error_msg = f"Неизвестный тариф: '{plan_input}'. Доступные: {available_plans}"
        ...
        plans = {
            "lite": {
                ...
                "attempts": 10, 
                "title": "LITE", 
                "description": "10 попыток проверки фото"
            },
            "premium": {
                "price": 1, 
                "attempts": 30, 
                "title": "PREMIUM", 
                "description": "30 попыток проверки фото"
            },
            "ultra": {
                "price": 1, 
                "attempts": 999999, 
                "title": "ULTRA", 
                "description": "Безлимит на 6 месяцев"
            }
        }
        
        plan_data = plans[normalized_plan]
        ...
        c.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (str(telegram_id), normalized_plan, plan_data["price"], "pending"))
        transaction_id = c.lastrowid
        conn.commit()
        conn.close()
        
        print(f"✅ Payment request: user={telegram_id}, plan={normalized_plan}, price={plan_data['price']}, transaction_id={transaction_id}")
